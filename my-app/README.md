...
**잘된 점**
- 물·땅·숲·빛이라는 브랜드 콘셉트를 페이지 구조와 인터랙션까지 일관되게 끌고 간 것
- 장바구니 → 결제(Polar) → 완료 페이지까지 실제 쇼핑몰 수준의 구매 흐름을 끝까지 구현한 것
- 모바일 터치 이슈(첫 탭 hover, pull-down 프리즈 등)를 임시방편이 아니라 원인을 찾아 패턴으로 해결한 것

**아쉬운 점**
- 상품 데이터를 JSON 파일로 관리해서 상품 추가·수정 시 코드 배포가 필요한 구조인 점
- 성능 측정(Lighthouse)과 최적화를 수치로 검증하지 못하고 마무리한 점
- 주문 완료 후 주문 내역 조회까지는 구현하지 못한 점

**다음에 시도할 것**
- 상품 데이터를 Firestore나 CMS로 옮겨 배포 없이 관리 가능한 구조로 개선
- TypeScript 도입으로 상품 옵션·장바구니 데이터 타입 안정성 확보
- Lighthouse 기준 성능 측정 후 이미지 최적화(WebP, lazy loading) 수치로 검증
